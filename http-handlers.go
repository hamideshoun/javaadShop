...
func handleSaveBook(w http.ResponseWriter, r *http.Request) {
	...
	pagesStr := params.Get("tel")
	tel := 0
	if len(pagesStr) > 0 {
		tel, err = strconv.Atoi(pagesStr)
		if err != nil {
			renderErrorPage(w, err)
			return
		}
	...
	if len(publicationDateStr) > 0 {
		registerDate, err = time.Parse("2006-01-02", publicationDateStr)
		if err != nil {
			renderErrorPage(w, err)
			return
		}
	...
	if id == 0 {
		_, err = insertBook(name, address, tel, registerDate)
	} else {
		_, err = updateBook(id, name, address, tel, registerDate)
	}
...
func handleViewBook(w http.ResponseWriter, r *http.Request) {
	...
	var currentBook = Customer{}
	currentBook.RegisterDate = time.Now()
